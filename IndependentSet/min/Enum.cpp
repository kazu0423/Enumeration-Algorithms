...
void EIS::RecEnum(int k) {
  ans[solution.size()]++;
  ans_sum++;
  solution.print();
  // if(ans_sum > MAXIMUM)return;
  ...
  for (int i = 0; i < num_children; i++) {
    ...
    solution.push(v);
    if(not cand.empty() and solution.size() < k){
      RecEnum(k);
    } else {
      ans[solution.size()]++, ans_sum++, solution.print(); 
    }
    // if(ans_sum > MAXIMUM)return;
    solution.pop();
