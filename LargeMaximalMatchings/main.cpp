...
void omputeMatched(Graph &G, boost::dynamic_bitset<> &m, std::vector<int> &matched){
    ...
    for(int j = 0; j < G[i].size(); j++) {
      if(matched[i] == -1 and matched[G[i][j]] == -1) {
        ...
        for(int k = 0; k < G.edge_size(); k++) {
          if(m.test(k)){
            std::cout << "{" << G.getEdge(k).first << " " << G.getEdge(k).second << "}" << std::endl;
          }
        }
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::vector<int> > e_to_id(n, std::vector<int>(n, -1));
  std::vector<std::pair<int, int> >id_to_e(m);
  ...
  while (getline(ist, tmp)) {
    ...
    id_to_e[id] = std::pair<int, int>(u, v);
    id++;
  }
  ...
  for(boost::tie(vi, vi_end) = vertices(GB); vi != vi_end; ++vi){
    if (mate[*vi] != boost::graph_traits<GraphB>::null_vertex() && *vi < mate[*vi]){
